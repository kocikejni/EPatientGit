    ...
    public class DoctorController : Controller
    {
        ...
        public ActionResult Index(string message)
        {
            ...
            var model = new CollectionOfAll
            {
                ...
                ActiveAppointments = db.Appointments.Include(c => c.Doctor).Include(c => c.Patient).Where(c => c.DoctorId == doctor.Id).Where(c => c.Status == true).Where(c => c.AppointmentDate >= date).ToList(),
                PendingAppointments = db.Appointments.Include(c => c.Doctor).Include(c => c.Patient).Where(c => c.DoctorId == doctor.Id).Where(c => c.Status == false).Where(c => c.AppointmentDate >= date).ToList()
        };
            return View(model);
        }
        ...
        public ActionResult AddPrescription(PrescriptionCollection model)
        {
            ...
            var patientuser = db.Users.SingleOrDefault(c => c.Id == patient.ApplicationUserId);
        ...
        public ActionResult PrintPartialViewOfPrescriptionToPdf(int id)
        {
            using (ApplicationDbContext db = new ApplicationDbContext())
            {
                ...
                var report = new PartialViewAsPdf("~/Views/Doctor/PrescriptionView.cshtml", prescription);
                return report;
            }
        }
        //Edit Prescription
